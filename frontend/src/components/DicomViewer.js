...
const DicomViewer = ({ imagenes = [], ajustesIniciales = {}, onCambioAjustes = null, estiloContenedor = {} }) => {
    ...
    const loadImage = useCallback(async (imgData) => {
        ...
        const fullUrl = (url.startsWith('http') || url.startsWith('blob:') || url.startsWith('data:')) ? url : window.location.origin + url;

        // Try DICOM loader first (wadouri is the standard for HTTP DICOM)
        const ids = ['wadouri:' + fullUrl];

        // For non-DICOM extensions, also try as web image
        const isWebImage = /\.(jpg|jpeg|png|gif|webp|bmp)(\?|$)/i.test(fullUrl);
        if (!isWebImage) {
            ids.push('dicomweb:' + fullUrl);
        }
        ...
        // Fallback: try fetching the file and checking if it's actually accessible
        try {
            ...
            if (!testResp.ok) {
                setErr(`No se pudo acceder al archivo (HTTP ${testResp.status}). Verifique que el archivo existe en el servidor.`);
            } else {
                setErr('El archivo existe pero no se pudo interpretar como DICOM. Verifique que es un archivo .dcm valido.');
            }
        } catch (fetchErr) {
            setErr('No se pudo conectar al servidor para cargar la imagen. Verifique la conexion.');
        }
